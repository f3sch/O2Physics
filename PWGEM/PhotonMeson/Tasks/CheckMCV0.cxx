...
struct CheckMCV0 {
  ...
  AxisSpec axisRMCDiff{xNBins, -50, 50, "#delta(R_{reco} - R_{MC})"};
  AxisSpec axisXMCDiff{xNBins, -50, 50, "#delta(X_{reco} - X_{MC})"};
  AxisSpec axisYMCDiff{yNBins, -50, 50, "#delta(Y_{reco} - Y_{MC})"};
  AxisSpec axisZMCDiff{zNBins, -50, 50, "#delta(Z_{reco} - Z_{MC})"};
  ...
  void processMCV0(CollisionsMC const& collisions, V0s const& v0s, FilteredTracksMC const& /*unused*/, aod::McParticles const& /*unused*/, aod::McCollisions const& /*unused*/, aod::BCsWithTimestamps const& /*unused*/)
  {
    ...
    for (auto& collision : collisions) {
      ...
      for (const auto& v0 : v0s.sliceBy(perCollision, static_cast<int>(collision.globalIndex()))) {
        ...
        float dr = v0.v0radius() - std::sqrt(posMC.vx() * posMC.vx() + posMC.vy() * posMC.vy());
